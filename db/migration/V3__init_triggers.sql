...
BEGIN
    IF (TG_OP = 'UPDATE') THEN
        INSERT INTO account_log (account_id, operation)
...
BEGIN
    IF (SELECT count(*)
        FROM profile
        WHERE profile.account_id = NEW.account_id) > 4 THEN
        RAISE EXCEPTION 'Cannot add more than 4 profiles to the account with ID: %', NEW.account_id;
    END IF;
...
BEGIN
    IF EXISTS (SELECT 1
               ...
               WHERE profile_id = NEW.profile_id
                 AND (movie_id = NEW.movie_id OR series_id = NEW.series_id)) THEN
